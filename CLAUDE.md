...
### Git 커밋 규칙
- **작업 단위 커밋**: 하나의 논리적 작업 완료 시마다 커밋 (기능, 버그픽스, 리팩토링 등)
- **커밋 메시지**: 한글로 작성
- **커밋 타입** (Conventional Commits 스타일):
  - `feat`: 새 기능 추가
  - `fix`: 버그 수정
  ...
  - `chore`: 빌드, 설정 파일 수정
  - `test`: 테스트 코드 추가/수정
  - `infra`: 인프라 관련 (Docker, Nginx 등)
- **커밋 메시지 예시**:
  ```
  feat: 서비스 신청 폼 유효성 검사 추가
  fix: 파트너 목록 페이지네이션 오류 수정
  refactor: API 클라이언트 모듈 구조 개선
  ```
- **제외 항목**: `.claude/` 디렉토리는 커밋하지 않음 (개인 설정)
