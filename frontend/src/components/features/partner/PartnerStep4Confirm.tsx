...
export function PartnerStep4Confirm({
  ...
  // 선택된 서비스 이름 가져오기 (코드 → 한글 이름 변환)
  const getSelectedServiceNames = (): string[] => {
    const names: string[] = [];
    categories.forEach((cat) => {
      cat.services.forEach((service) => {
        if (values.serviceAreas?.includes(service.code)) {
  ...
  const confirmationSections: ConfirmationSection[] = [
    ...
    },
    {
      ...
      items: [
        {
          label: "회사/본인 소개",
          value: values.introduction || "미입력",
          icon: <FileText className="w-5 h-5" />,
        },
        {
          label: "경력 및 자격",
          value: values.experience || "미입력",
        },
        {
          label: "비고",
          value: values.remarks || "미입력",
        },
      ],
    },
  ];
