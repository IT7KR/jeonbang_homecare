...
// 배정 상태 설정
const ASSIGNMENT_STATUS_CONFIG: Record<string, { label: string; color: string; bgColor: string }> = {
...
export default function PartnerViewPage() {
          ...
          <CardContent className="pt-8 pb-8">
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <div className="pt-4 space-y-2 text-sm text-gray-500">
                <p>문의가 필요하시면</p>
                <a href="tel:031-XXX-XXXX" className="flex items-center justify-center gap-2 text-primary font-medium">
  ...
  const statusConfig = ASSIGNMENT_STATUS_CONFIG[data.assignment_status] || ASSIGNMENT_STATUS_CONFIG.pending;
      ...
      <div className="container max-w-lg mx-auto px-4 py-4 space-y-4">
            ...
            {data.scheduled_date ? (
              <div className="flex items-center gap-3 p-3 bg-green-50 rounded-xl">
                ...
                <div>
                  <p className="font-semibold text-gray-900">
                    ...
                    {data.scheduled_time && (
                      <span className="text-green-600 ml-2">{data.scheduled_time}</span>
                    )}
                  </p>
              ...
              <div className="space-y-3">
                <div className="p-3 bg-gray-50 rounded-xl">
                  <p className="text-gray-500 text-sm">일정이 아직 확정되지 않았습니다</p>
                </div>
                {(data.preferred_consultation_date || data.preferred_work_date) && (
                  <div className="grid gap-2">
                    {data.preferred_consultation_date && (
                      <div className="flex items-center gap-2 text-sm text-gray-600 bg-blue-50/50 px-3 py-2 rounded-lg">
                        ...
                        <span className="ml-auto font-medium">{formatShortDate(data.preferred_consultation_date)}</span>
                      </div>
                    )}
                    {data.preferred_work_date && (
                      <div className="flex items-center gap-2 text-sm text-gray-600 bg-orange-50/50 px-3 py-2 rounded-lg">
                        ...
                        <span className="ml-auto font-medium">{formatShortDate(data.preferred_work_date)}</span>
                      </div>
                    )}
                  </div>
                )}
          ...
          <CardContent className="pt-0 space-y-3">
            <div className="grid gap-2">
              <div className="flex items-center gap-3 p-2.5 bg-gray-50 rounded-lg">
                ...
                <span className="text-gray-700 font-medium">{data.customer_name_masked}</span>
              </div>
              <div className="flex items-center gap-3 p-2.5 bg-gray-50 rounded-lg">
            ...
            <CardContent className="py-4 space-y-3">
              ...
              {data.estimate_note && (
                <div className="pt-2 border-t">
                  ...
                  <p className="text-gray-700 whitespace-pre-wrap text-sm">{data.estimate_note}</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="border-0 shadow-md">
            ...
            <CardContent className="pt-0">
              <div className="p-3 bg-amber-50 rounded-xl border border-amber-100">
                <p className="text-gray-700 whitespace-pre-wrap text-sm">{data.note}</p>
              </div>
            </CardContent>
          </Card>
        )}
        ...
        <div className="text-center py-4 space-y-1">
          <p className="text-sm text-gray-400">
            이 링크는 <span className="font-medium text-gray-500">{formatDate(data.token_expires_at)}</span>까지 유효합니다
          </p>
